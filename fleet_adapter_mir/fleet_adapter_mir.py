...
class MiRCommandHandle(adpt.RobotCommandHandle):
    def __init__(self,
        ...
        self.state_update_timer = self.node.create_timer(
            1.0 / self.robot_state_update_frequency,
        ...
        def path_following_closure():
            ...
            while ((self.rmf_remaining_path_waypoints or self.paused)
                ...
                if not self.paused:  # Skipped if paused
                    # remaining time to reach the next waypoint
                    if _next_waypoint is not None:
                        waypoint_leave_msg = _next_waypoint.time
                ...
                if (next_mission_wait <= 0
                    ...
                    with self._update_mutex:
                        ...
                        #retry = 0
                        #print(f"Waiting for MIR to start executing the new task")
                        #while self.mir_state != MiRState.EXECUTING and retry < 10:
                        #    print(self.mir_state)
                        #    self.execute_updates()
                        #    sleep(1.0 / self.robot_state_update_frequency)
                        #    retry += 1
    ...
    def estimate_arrival(self, goal, current_position):
        ...
        # d_accel = self.linear_velocity ** 2 / 2 * self.linear_acceleration
        # t_accel = self.linear_velocity / self.linear_acceleration
        # if translation > 2 * d_accel:
        #    t_translation = 2 * t_accel +
    ...
    ##########################################################################
    def get_position(self, api_response=None, as_dimensions=False):
    ...
    def update_internal_robot_state(self, api_response=None):
        ...
        try:
            ...
            rmf_location.x, rmf_location.y, rmf_location.yaw = (
                self.get_position(api_response=api_response)
            )
